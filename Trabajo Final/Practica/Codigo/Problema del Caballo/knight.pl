...
%% successors([Lista],N,K0) Es verdadero si cada posicion del tablero unifica con la posicion a la que debería ir un caballo en esa coordenada
...
% K1 #= K0 + 1 Verdadero si K1 unifica con la suma aritmetica de k0 y 1.

/*
	El predicado successors principalmente analiza cuales son todas las posiciones validas y disponibles 
	desde la posicion inicial hasta la final para un caballo en un tablero de NxN dimensiones.
	
	Inicialmente se abriran varias posibilidades de movimientos para las primeras casillas, pero una vez el problema
	avance solo serán viables los caminos que cumplen que para todas las casillas el caballo tenga un movimiento posible.
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   ...
   No creemos necesaria la explicación de estas lineas de código que 
   simplemente muestran el camino por pantalla para el entendimiento 
   de la programación del problema.
- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
